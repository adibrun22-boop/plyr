...
export default function EditTrainingProgram() {
  ...
  const { data: program, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const programs = await base44.entities.TrainingProgram.filter({ id: programId });
      return programs[0];
    },
    enabled: !!programId,
  });
  ...
  useEffect(() => {
    if (program) {
      setFormData({
        ...
        sport_type: program.sport_type || 'football',
  ...
  const updateProgramMutation = useMutation({
    mutationFn: (data) => base44.entities.TrainingProgram.update(programId, data),
    onSuccess: () => {
      queryClient.invalidateQueries(['training-programs']);
      queryClient.invalidateQueries(['training-program', programId]);
      toast.success(isRTL ? 'תוכנית האימון עודכנה בהצלחה' : 'Training program updated successfully');
      navigate(createPageUrl('CoachDashboard'));
    },
  });

  const deleteProgramMutation = useMutation({
    mutationFn: () => base44.entities.TrainingProgram.delete(programId),
    onSuccess: () => {
      queryClient.invalidateQueries(['training-programs']);
      toast.success(isRTL ? 'תוכנית האימון נמחקה' : 'Training program deleted');
      navigate(createPageUrl('CoachDashboard'));
    },
  });
  ...
  const handleDelete = () => {
    if (confirm(isRTL ? 'האם אתה בטוח שברצונך למחוק תוכנית זו?' : 'Are you sure you want to delete this program?')) {
  ...
  if (isLoading) {
    return (
      <div className="max-w-4xl mx-auto p-6 text-center">
        <p className="text-gray-500">{isRTL ? 'טוען...' : 'Loading...'}</p>
      </div>
    );
  }

  if (player?.profile_type !== 'coach' || program?.coach_id !== player?.id) {
    return (
      <div className="max-w-4xl mx-auto p-6">
        <Card>
          <CardContent className="p-8 text-center">
            <p className="text-gray-500">
              {isRTL ? 'אין לך הרשאה לערוך תוכנית זו' : 'You do not have permission to edit this program'}
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className={cn("flex items-center justify-between mb-6", isRTL && "flex-row-reverse")}>
        <h1 className="text-3xl font-bold">
          {isRTL ? 'ערוך תוכנית אימון' : 'Edit Training Program'}
        </h1>
        <Button
          variant="destructive"
        ...
        >
          ...
          {isRTL ? 'מחק תוכנית' : 'Delete Program'}
        </Button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className={cn("flex gap-3", isRTL && "flex-row-reverse")}>
          <Button type="submit" className="bg-emerald-600 hover:bg-emerald-700" disabled={updateProgramMutation.isPending}>
